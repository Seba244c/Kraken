...
namespace Kraken {
    ...
    static uint32_t ShaderDataTypeSize(const ShaderDataType type) {
        switch (type) {
            ...
            default: KRC_ASSERT(false, "Unknown ShaderDataType!"); return 0;
        }
    }
    ...
    struct BufferElement {
        ...
        BufferElement(const ShaderDataType type, const std::string& name) : Name(name), Type(type), Size(ShaderDataTypeSize(type)), Offset(0), Normalized(false) {
        }
        ...
        [[nodiscard]] uint32_t GetComponentCount() const {
            ...
            KRC_ASSERT(false, "Unknown ShaderDataType!"); return 0;
        }
    };
